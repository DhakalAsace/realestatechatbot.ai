...
Latest verified branch preview:

```text
https://realestatechatbot-ai-git-phase-1-h-31b7fe-dhakalasaces-projects.vercel.app
```
...
- Phase 10 public chat hardening blocks oversized raw request bodies before JSON parsing and blocks link floods, unsafe markup, repeated characters, spam phrases, and prompt/secret extraction before channel resolution, sessions, usage, AI, messages, or leads are created. Abuse blocks are recorded for owner/admin review after channel resolution.
...
- Phase 10 added `/dashboard/admin` and `/dashboard/audit` for workspace-scoped launch readiness, audit review, abuse blocks, usage events, notification health, and all documented external launch gates. Bulk export and audit/abuse review are owner/admin-only.
...
- Phase 10 checks passed on AWS after final audit hardening: `npm run typecheck`, `npm run lint`, `npm test` (19 files, 91 tests after adding auth-confirm and raw payload-cap coverage), `npm run build`, `npm run test:bundle-secrets`, and `npm run test:e2e` (16 Chromium tests). The Phase 7 follow-up scheduler now chunks active message lookups to keep verification stable as test data grows.
...
- Current Phase 10 branch preview alias: https://realestatechatbot-ai-git-phase-1-h-31b7fe-dhakalasaces-projects.vercel.app. Vercel inspect status is verified after each final push. Authenticated `vercel curl` verifies `/privacy`, `/terms`, `/login`, `/c/sarah-patel`, and `/auth/confirm` fail-closed redirects.
